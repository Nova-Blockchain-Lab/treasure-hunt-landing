// Types for the public post-event report snapshot for the Data with Purpose
// Summit edition. Runtime payload lives in
// `data/datasummit2026/report-snapshot.json`.
//
// This is the trimmed NFC-only shape: no stages, no polls, no QR. The summit
// had no exhibitor booths, so the "exhibitors" concept from PSCS is replaced
// by player-owned **tags** plus top hiders & finders.
...
export async function readSnapshot(): Promise<Snapshot> {
  ...
  const file = path.join(process.cwd(), "data", "datasummit2026", "report-snapshot.json")
