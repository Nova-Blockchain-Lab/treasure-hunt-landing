...
├── media-section.tsx       # Interview video + pull-quote + CSS-columns masonry gallery (6 event photos + 2 vertical clips, natural aspect; one landscape shot gets a `box` aspect to cap a column); assets in public/media/; id="media". interview.mp4 is the full 2:26 Smart Cities Summit interview (PT audio, burned-in EN subtitles), sourced from ../insta/"Treasure Hunt 1.0.mov" and processed with the pipeline in that scratchpad (whisper transcript → hand PT→EN translation → PIL-rendered subtitle PNGs overlaid via ffmpeg, since this ffmpeg has no libass; 4K→720p). English SRT kept at public/media/interview.en.srt.
