...
├── media-section.tsx       # Two interview videos side by side (equal cols) + centered pull-quote band + CSS-columns masonry gallery (6 event photos + 2 vertical clips, natural aspect; one landscape shot gets a `box` aspect to cap a column); assets in public/media/; id="media". Video 1 (`watchInterview` badge "Full interview") = interview.mp4, the full 2:26 Smart Cities Summit interview (PT audio, burned-in EN subtitles), sourced from ../insta/"Treasure Hunt 1.0.mov" and processed with the pipeline in that scratchpad (whisper transcript → hand PT→EN translation → PIL-rendered subtitle PNGs overlaid via ffmpeg, since this ffmpeg has no libass; 4K→720p; English SRT at public/media/interview.en.srt). Video 2 (`interviewTeaser` badge "Highlights") = interview-teaser.mp4, the original 27s branded highlights cut (Dinis Araújo lower-third + EN subs already baked in).
